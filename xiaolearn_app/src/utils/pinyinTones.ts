...
/**
 * spacePinyin — insère des espaces entre les syllabes pinyin quand ils manquent.
 * Algorithme : chaque voyelle tonée (avec diacritique) marque la fin du noyau
 * d'une syllabe ; on consomme ensuite les voyelles atones finales (diphtongues),
 * la coda -n/-ng/-r, puis on insère un espace avant la syllabe suivante.
 *
 * Cas non couverts : syllabes à ton neutre (de, ma, ne…) sans diacritique.
...
export function spacePinyin(raw: string): string {
  ...
  while (i < raw.length) {
    ...
    if (_TONED.has(ch)) {
      let j = i + 1;
      // Diphtongue : consommer les voyelles atones qui suivent (même syllabe)
      ...
      // Espace avant la syllabe suivante (sauf ponctuation)

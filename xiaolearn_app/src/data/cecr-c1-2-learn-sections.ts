...
export const c12ConvM1LearnSections: LessonV2LearnSection[] = [
  ...
  },
  {
    ...
    body:
      'Structure d\'un contrat 合同 : 甲方 (Partie A), 乙方 (Partie B), 标的 (objet du contrat), 价款 (prix), 期限 (durée), 违约责任 (responsabilité en cas de rupture), 不可抗力 (force majeure), 争议解决 (règlement des différends). Phrases pour clarifier : 这条款的具体含义是什么 ? (quel est le sens précis de cette clause ?). Demander modif : 我希望在第 X 条加上 Y (j\'aimerais ajouter Y à la clause X). Astuce : sécuriser avec 我建议增加一条不可抗力条款 (ajoutons une clause force majeure). Règle : conclure par 这份合同我需要带回去研究 (je dois étudier ce contrat plus en détail) — Attention : ne jamais signer sur place.',
    bodyEn:
...
export const c12ConvM2LearnSections: LessonV2LearnSection[] = [
  {
    ...
    body:
      'Pré-négo : connaître la 关系 (réseau) de l\'autre + son 面子 (face publique). RÈGLE D\'OR : éviter le « non » direct — préfère 这个我们可以再考虑 (on peut reconsidérer) ou 这个有点困难 (c\'est un peu difficile) qui SIGNIFIENT NON sans le dire. Astuce : lire les signaux — 让我们再研究一下 (étudions ça encore) = souvent un « non » poli. Pour pousser : 我们的诚意是希望长期合作 (notre sincérité est de viser une collaboration long-terme). Remarque : 长期 (chángqī, long-terme) est le mot-clé business chinois. Règle : ne pas conclure trop vite — un déjeuner / dîner après la négo formelle est obligatoire pour sceller le 关系.',
    bodyEn:
  ...
  },
  {
    ...
    body:
      'Structure : 背景 → 问题 → 战略方案 → 预期效果 → 风险 → 行动计划. Phrases d\'ouverture : 各位领导，我今天向大家汇报 X 的战略方案 (je présente à vous tous le plan stratégique X). Présenter une donnée : 据统计 X (selon les statistiques X). RÈGLE D\'OR : argumenter avec 之所以选择这个方案，是因为 X (la raison de ce plan c\'est X). Astuce : anticiper l\'objection avec 有人可能担心 X，但是 Y. Conclure : 我希望各位领导能给予支持 (j\'espère votre soutien). Remarque : 给予支持 (donner du soutien) = formule attendue de demande d\'approbation hiérarchique.',
    bodyEn:
...
export const c12ConvM3LearnSections: LessonV2LearnSection[] = [
  {
    ...
    body:
      'Cadre : prise de parole publique sur un sujet de société. Règle : ouvrir solennellement par 各位朋友，今天我想跟大家分享 X 的看法. Énoncer un constat : 我们身边正发生着 X 的现象 (autour de nous se produit le phénomène X). Mobiliser les valeurs : 我们追求的是 X (ce à quoi nous aspirons est X). Verbes-clés : 倡导 (chàngdǎo, prôner), 呼吁 (hūyù, appeler à), 推动 (tuīdòng, faire avancer). Astuce : combo motivationnel 让我们携手 X (joignons les mains pour X — 携手 est très soutenu). Conclure : 让我们共同努力，把这个梦想变成现实 (transformons ce rêve en réalité). Attention : ton SOUTENU mais pas lyrique outre-mesure.',
    bodyEn:
  ...
  },
  {
    ...
    body:
      'Structure : 我们的使命 (mission) → 主要工作 (activités) → 已取得的成果 (résultats) → 未来的计划 (futur). Vocab : 公益 (gōngyì, intérêt général), 慈善 (císhàn, charité), 志愿者 (zhìyuànzhě, bénévole), 募捐 (mùjuān, collecte de fonds). Règle : 我们致力于 X (nous nous consacrons à X — 致力于 est soutenu). Astuce : pour mobiliser, 我们诚邀您加入我们的行列 (nous vous invitons sincèrement à rejoindre nos rangs — 诚邀 + 行列 = très formel). Témoigner d\'impact : 我们已经帮助了 X 个人 (nous avons aidé X personnes). RÈGLE D\'OR : conclure par 您的支持，意味着 X (votre soutien signifie X — formule percutante).',
    bodyEn:
...
export const c12ConvM4LearnSections: LessonV2LearnSection[] = [
  ...
  },
  {
    ...
    body:
      'Préparer le terrain : 我有一件事要跟您谈，希望您冷静听 (j\'ai une chose à vous dire, restez calme). Annoncer : 公司决定与您解除劳动合同 (la société a décidé de résilier votre contrat). Règle : justifier par 由于 X 的原因 (pour la raison X — toujours invoquer une cause). Astuce : adoucir avec 这不是您个人能力的问题 (ce n\'est pas un problème de capacité personnelle). Vocab : 解除合同 (résilier), 补偿金 (indemnité), 离职手续 (formalités de départ), 推荐信 (lettre de recommandation). RÈGLE D\'OR : toujours offrir 我们可以为您提供推荐信 (nous pouvons fournir une recommandation). Remarque : conclure par 我个人非常感谢您过去的贡献 (personnellement, merci pour vos contributions passées) — humanise le moment.',
    bodyEn:
...
export const c12ConvM5LearnSections: LessonV2LearnSection[] = [
  {
    ...
    body:
      'Structure 10 mins : 团队 → 痛点 → 解决方案 → 市场 → 商业模式 → 数据 → 融资计划. Vocab : 估值 (gūzhí, valorisation), 融资 (róngzī, levée de fonds), 商业模式 (shāngyè móshì, business model), 现金流 (xiànjīn liú, cashflow), 退出机制 (tuìchū jīzhì, sortie/exit). RÈGLE D\'OR : pitch d\'ouverture 我们解决的痛点是 X (le pain point qu\'on résout est X). Remarque : 痛点 (tòngdiǎn, pain point) est un mot-clé obsessionnel des VC chinois. Forces : 我们的护城河是 X (notre moat est X — métaphore venue de Buffett, 护城河 = douve). Conclure : 我们希望融资 X 万美元 (nous cherchons à lever X). Attention : le VC chinois aime VOIR DES CHIFFRES rapidement.',
    bodyEn:
  ...
  },
  {
    ...
    body:
      'Structure : 风险描述 → 影响评估 → 当前应对措施 → 建议. Phrases : 我必须向各位董事报告一个潜在风险 (je dois rapporter aux directeurs un risque potentiel). Remarque : 潜在 (qiánzài, latent/potentiel) = mot-clé de gouvernance. Quantifier : 影响可能在 X 之间 (impact entre X). Règle : mesures avec 我们已经采取的措施包括 X (les mesures déjà prises incluent X). Demande : 我建议董事会授权 X (je recommande au CA d\'autoriser X). Astuce : 授权 (shòuquán, autoriser) est le verbe-clé de gouvernance. Conclure : 我会随时向董事会汇报进展 (je tiendrai le CA informé). Attention : ton sérieux, factuel, jamais alarmiste.',
    bodyEn:
...
export const c12ConvM6LearnSections: LessonV2LearnSection[] = [
  {
    ...
    body:
      'Cadre : différend pro entre département A (ex : ventes) et département B (ex : R&D). Position : 我作为公司高层，希望从全局角度看这个问题 (en tant que dirigeant, je vois ça d\'un point de vue global). Remarque : 全局 (quánjú, perspective d\'ensemble) est le mot-clé. Reformuler : 销售部担心的是 X，研发部坚持的是 Y. Règle : identifier le bien commun avec 双方的目标都是公司的发展 (votre objectif commun est la croissance). Astuce : proposer 我们能否找一个折中方案，既满足 X 又考虑 Y ? RÈGLE D\'OR : décision avec 经过权衡，我决定 X (après pesée, je décide X). Attention : 权衡 (quánhéng, peser le pour et le contre) montre la délibération.',
    bodyEn:
  ...
  },
  {
    ...
    body:
      'Ouvrir : 各位来自世界各地的朋友，大家好 (chers amis du monde entier). RÈGLE D\'OR : mention culturelle 古人云 X (les anciens disaient X — citation classique sécurise la crédibilité). Articuler la thèse : 今天我想分享 X 给我们的启示 (aujourd\'hui je partage les enseignements de X). Remarque : 启示 (qǐshì, enseignement, révélation) est plus soutenu que 教训 (leçon). Exemples : 让我举一个例子 X. Astuce : verbes solides 阐述 (chǎnshù, exposer en détail), 揭示 (jiēshì, révéler). Conclure : 让我们记住 X (souvenons-nous de X). 谢谢大家的聆听 + bow léger. Attention : pour traduction simultanée, ralentir + pauses entre groupes de 8-10 mots = aide énorme aux interprètes.',
    bodyEn:
...
export const c12ConvM7LearnSections: LessonV2LearnSection[] = [
  {
    ...
    body:
      'Structure : 摘要 (résumé) → 引言 (introduction) → 现状 (état des lieux) → 分析 (analyse) → 政策建议 (recommandations) → 结论 (conclusion) → 参考文献 (références). Règle : phrase d\'ouverture 本白皮书旨在探讨 X (ce livre blanc vise à explorer X). Remarque : 本 (běn, ce / présent) + nom = formule consacrée du langage administratif/académique. Recommandations : 我们呼吁有关部门采取以下措施 (nous appelons les autorités à prendre les mesures suivantes). Conclure : 我们期待与社会各界共同推动 X 的发展 (nous espérons faire avancer X avec toute la société). Astuce : ton dense, impersonnel, factuel. RÈGLE D\'OR : pas de « 我 » dans un livre blanc — préfère 笔者 (l\'auteur) ou « 本研究 ».',
    bodyEn:
  ...
  },
  {
    ...
    body:
      'Cadre : adresse collective à une autorité (gouvernement, entreprise, université) sur un sujet d\'intérêt public. Règle : ouverture par 致 X (à X) — formule épistolaire formelle. Cause : 我们，作为 X，对 Y 表示深切的关注 (nous, en tant que X, exprimons notre profonde préoccupation pour Y). Remarque : 深切 (shēnqiè, profond) marque le sérieux. Demandes : 我们呼吁 X / 我们要求 X. RÈGLE D\'OR : hiérarchie 呼吁 (appel) < 请求 (demande) < 要求 (exigence) < 强烈要求 (exigence forte). Conclure : 我们期待您能积极回应我们的诉求 (nous espérons une réponse active à nos demandes). Signer : nom + titre + organisation. Astuce : pour pétition à plusieurs, 此致敬礼 + nom collectif (« 100 名 X 联署 » — co-signé par 100 X).',
    bodyEn:
...
export const c12NuancesM1LearnSections: LessonV2LearnSection[] = [
  {
    ...
    body:
      'Règle : 实现 (shíxiàn) = réaliser un rêve / objectif abstrait. 实现梦想 / 实现目标. Remarque : connote la projection dans le futur qui devient présent — verbe noble. Règle : 完成 (wánchéng) = achever une tâche concrète délimitée. 完成任务 / 完成作业. Remarque : connote la fin d\'une activité avec un délai. Règle : 达成 (dáchéng) = parvenir à un accord/consensus, mutuellement avec un autre. 达成协议 / 达成共识. Remarque : connote la collaboration et le résultat négocié. Astuce : hiérarchie 完成 (concret délimité) < 实现 (abstrait noble) < 达成 (collaboratif). Attention : erreur classique 完成梦想 ✗ → 实现梦想 ✓.',
    bodyEn:
  ...
  },
  {
    ...
    body:
      'Règle : 建议 (jiànyì) = suggestion (le plus universel, oral et écrit). 我建议你 X. Remarque : adresse à un individu ou un groupe. Règle : 提议 (tíyì) = proposition formelle (en réunion, vote). 我提议我们投票 (je propose qu\'on vote). Remarque : plus structuré, parfois mis aux voix. RÈGLE D\'OR : 倡议 (chàngyì) = initiative publique (lancement d\'une campagne, d\'un mouvement). 倡议节约用水 (lancer une initiative pour économiser l\'eau). Remarque : connote la portée collective et symbolique. Astuce : hiérarchie 建议 (universel) < 提议 (réunion formelle) < 倡议 (initiative publique). Attention : « 一带一路倡议 » (l\'initiative Belt and Road) — pourquoi 倡议 et non 建议 ? Échelle géopolitique.',
    bodyEn:
...
export const c12NuancesM2LearnSections: LessonV2LearnSection[] = [
  {
    ...
    body:
      'Règle : 保持 (bǎochí) = garder un état (qualité, niveau). 保持安静 (rester silencieux), 保持联系 (rester en contact). Remarque : le plus universel. Règle : 维持 (wéichí) = maintenir malgré les difficultés (effort actif). 维持秩序 (maintenir l\'ordre), 维持生活 (maintenir son train de vie). Remarque : connote l\'effort pour empêcher la chute. RÈGLE D\'OR : 维护 (wéihù) = protéger, défendre activement. 维护权利 (défendre des droits), 维护和平 (défendre la paix). Remarque : connote la défense contre une menace. Astuce : hiérarchie 保持 (état stable) < 维持 (effort actif anti-chute) < 维护 (défense contre menace). Attention : erreur classique 保持权利 ✗ → 维护权利 ✓ (les droits se défendent, ne se gardent pas passivement).',
    bodyEn:
  ...
  },
  {
    ...
    body:
      'Règle : 破坏 (pòhuài) = détruire / endommager (le plus universel). 破坏环境 (détruire l\'environnement), 破坏关系 (briser une relation). Remarque : réversible souvent. RÈGLE D\'OR : 摧毁 (cuīhuǐ) = détruire totalement (effacer, raser). 摧毁建筑 (raser un bâtiment), 摧毁信心 (anéantir la confiance). Attention : irréversible, brutal. Règle : 损害 (sǔnhài) = nuire à, porter préjudice (effet sur la valeur, l\'intérêt). 损害健康 (nuire à la santé), 损害利益 (nuire aux intérêts). Remarque : connote le préjudice plutôt que la destruction physique. Astuce : hiérarchie 损害 (préjudice partiel) < 破坏 (destruction normale) < 摧毁 (destruction totale).',
    bodyEn:
...
export const c12NuancesM3LearnSections: LessonV2LearnSection[] = [
  {
    ...
    body:
      'Règle : 提出 (tíchū) = soulever / proposer (idée, question). 提出建议 / 提出问题. Remarque : action neutre de mettre en avant. RÈGLE D\'OR : 提倡 (tíchàng) = prôner, encourager (un comportement / une valeur). 提倡环保 (encourager la protection de l\'environnement), 提倡阅读. Remarque : connotation morale / sociale. Règle : 提升 (tíshēng) = élever (un niveau, une position). 提升能力 / 提升地位. Remarque : connotation de progression quantifiable. Astuce : test « soulever une idée » → 提出 ; « encourager un comportement » → 提倡 ; « augmenter un niveau » → 提升. Attention : trois verbes en 提 + composant qui changent radicalement le sens.',
    bodyEn:
  ...
  },
  {
    ...
    body:
      'Règle : 功能 (gōngnéng) = fonctionnalité d\'un objet/système (ce qu\'il peut faire). 这部手机有很多功能 (ce téléphone a beaucoup de fonctions). Remarque : concret, technique. Règle : 功用 (gōngyòng) = utilité pratique d\'un outil ou méthode (ce à quoi ça sert). 这种药的功用是 X (ce médicament sert à X). Remarque : plus formel. Règle : 用途 (yòngtú) = usage / destination (à quoi c\'est utilisé). 这房间的用途是 X (l\'usage de cette pièce est X). Remarque : connote la catégorie d\'emploi. Astuce : hiérarchie 功能 (capacités multiples) ≠ 功用 (utilité spécifique) ≠ 用途 (destination/catégorie). Astuce : test « il a 5 X » → 功能 ; « son X est de soigner » → 功用 ; « son X est résidentiel » → 用途.',
    bodyEn:
...
export const c12NuancesM4LearnSections: LessonV2LearnSection[] = [
  {
    ...
    body:
      'Règle : 方便 (fāngbiàn) = pratique / commode (le plus universel, oral et écrit). 这里很方便 / 您方便吗 ?. Remarque : le plus large. Attention : 方便 = euphémisme pour aller aux toilettes (mafan : 我去方便一下). Règle : 便利 (biànlì) = commodité (nom : un service / installation pratique). 便利店 (convenience store), 提供便利. Remarque : plus écrit/marketing. RÈGLE D\'OR : 便捷 (biànjié) = pratique ET rapide (combine commodité + efficacité). 便捷的支付方式 (mode de paiement pratique et rapide). Remarque : connote la technologie / digital. Astuce : hiérarchie 方便 (universel) < 便利 (commodité service) < 便捷 (rapide+pratique tech). Remarque : en 2026, 便捷 est devenu le mot-clé du marketing digital chinois.',
    bodyEn:
  ...
  },
  {
    ...
    body:
      'Règle : 依据 (yījù) = se baser sur / référer à (un fait, une norme, une règle). 依据法律 X (selon la loi X). Remarque : plus formel/juridique. Astuce : souvent + nom de référence. RÈGLE D\'OR : 凭借 (píngjiè) = grâce à (un atout, une compétence, une qualité). 凭借自己的能力 / 凭借努力. Remarque : connote l\'instrument du succès — plus écrit. Règle : 依靠 (yīkào) = compter sur / s\'appuyer sur (une personne, un soutien). 依靠父母 / 依靠政府. Remarque : connote dépendance/soutien — plus oral et émotionnel. Astuce : hiérarchie 依据 (référence formelle) ≠ 凭借 (atout personnel) ≠ 依靠 (soutien dépendant). Attention : « 凭借自己的努力获得成功 » = grâce à ses propres efforts (vertu de l\'autonomie chinoise).',
    bodyEn:
...
export const c12NuancesM5LearnSections: LessonV2LearnSection[] = [
  {
    ...
    body:
      'Règle : 减少 (jiǎnshǎo) = réduire en quantité (le plus universel). 减少污染 / 减少开支. Règle : 减轻 (jiǎnqīng) = alléger (un poids, une douleur, un fardeau). 减轻负担 (alléger le fardeau), 减轻痛苦 (soulager la douleur). Remarque : connote le soulagement. RÈGLE D\'OR : 削减 (xuējiǎn) = sabrer, couper de manière drastique (budgets, postes). 削减预算 (couper le budget), 削减人员 (couper les effectifs). Attention : verbe brutal, souvent négatif. Astuce : hiérarchie 减轻 (alléger doux) < 减少 (réduire neutre) < 削减 (sabrer brutal). Remarque : en éditorial éco, 削减 marque la rigueur ; 减轻 marque la compassion ; 减少 reste neutre/technique.',
    bodyEn:
  ...
  },
  {
    ...
    body:
      'Règle : 增加 (zēngjiā) = augmenter une quantité (nombre, somme). 增加人手 / 增加预算. Remarque : le plus universel quantitatif. Règle : 提高 (tígāo) = élever un niveau (qualité, capacité, prix). 提高效率 / 提高价格 / 提高生活质量. Remarque : sur l\'échelle qualitative. RÈGLE D\'OR : 增长 (zēngzhǎng) = croître de manière continue dans le temps (économique, démographique). 经济增长 / 人口增长. Remarque : connote la progression soutenue. Astuce : hiérarchie 增加 (quantité ponctuelle) ≠ 提高 (niveau qualitatif) ≠ 增长 (croissance continue). Attention : erreur 提高人口 ✗ → 增长人口 ✓ ou 增加人口 ✓. Remarque : la population a une croissance, pas une qualité élevée.',
    bodyEn:
...
export const c12NuancesM6LearnSections: LessonV2LearnSection[] = [
  {
    ...
    body:
      'Règle : 来往 (láiwǎng) = aller et venir (mouvements physiques OU contacts sociaux). 街上来往的人很多 (beaucoup de gens vont et viennent). Remarque : 我和他没有来往 = je n\'ai pas de contact avec lui. RÈGLE D\'OR : 往来 (wǎnglái) = échanges (formel, surtout commercial / diplomatique). 商业往来 (relations commerciales), 国家间的往来 (échanges entre pays). Remarque : plus formel. Règle : 交往 (jiāowǎng) = fréquentation sociale, relation suivie (avec l\'idée de durée). 他们交往多年 (ils se fréquentent depuis des années). Attention : 交往 = sortir avec qqn (couple). Astuce : hiérarchie 来往 (mouvement / contact ponctuel) < 交往 (relation suivie) < 往来 (formel commercial/diplomatique).',
    bodyEn:
  ...
  },
  {
    ...
    body:
      'Règle : 经历 (jīnglì) = expérience vécue (les événements traversés). 这是一段难忘的经历 (ce fut une expérience inoubliable). Remarque : concret, événementiel — verbe : avoir vécu. Règle : 经验 (jīngyàn) = expérience accumulée (savoir-faire). 我有 5 年的经验 (j\'ai 5 ans d\'expérience). Remarque : compétence professionnelle. RÈGLE D\'OR : 阅历 (yuèlì) = expérience de la vie (sagesse, recul, connaissance du monde). 他阅历丰富 (il a beaucoup vécu, du recul). Remarque : plus écrit, plus noble — connote la sagesse accumulée par le vécu, pas que pro. Astuce : hiérarchie 经历 (événement vécu) < 经验 (savoir-faire pro) < 阅历 (sagesse vie large).',
    bodyEn:
...
export const c12NuancesM7LearnSections: LessonV2LearnSection[] = [
  {
    ...
    body:
      'Règle : 质量 (zhìliàng) = qualité matérielle (produit, ouvrage). 这件衣服质量很好 (cette chemise est de bonne qualité). Remarque : le plus universel pour un objet/produit. Règle : 品质 (pǐnzhì) = qualité intrinsèque / morale (objet noble OU caractère humain). 高品质的产品 (produit haut de gamme), 优秀的品质 (excellentes qualités morales). Remarque : plus marketing/moral. RÈGLE D\'OR : 素质 (sùzhì) = qualité humaine (éducation, comportement civique). 国民素质 (qualité civique des citoyens), 提高素质. Remarque : connote le niveau social/civique. Astuce : hiérarchie 质量 (matériel) < 品质 (noble objet/morale) < 素质 (humain civique). Attention : erreur 这个人质量好 ✗ → 这个人素质高 ✓ (la qualité humaine est 素质).',
    bodyEn:
  ...
  },
  {
    ...
    body:
      'Règle : 一点 (yìdiǎn) = un peu (neutre, oral et écrit). 我有一点累. Remarque : aussi en négatif — 一点也不累 = pas du tout fatigué. Astuce : universel. RÈGLE D\'OR : 丝毫 (sīháo) = le moindre (presque toujours en négatif, écrit). 我没有丝毫的怀疑 = je n\'ai pas le moindre doute. Remarque : plus emphatique que 一点. RÈGLE D\'OR : 毫无 (háowú) = n\'avoir pas du tout (négation absolue, écrit). 毫无道理 (n\'a aucun sens), 毫无希望 (sans aucun espoir). Règle : 毫无 + nom abstrait. Astuce : hiérarchie 一点 (neutre oral) < 丝毫 (insistant écrit, surtout négatif) < 毫无 (absolu, abstrait). Remarque : en écrit C1+, 毫无 et 丝毫 marquent la maîtrise lexicale du registre soutenu.',
    bodyEn:
...
export const c12ChengyuAdvancedM1LearnSections: LessonV2LearnSection[] = [
  {
    ...
    body:
      '三顾茅庐 (« 3 visites à la chaumière ») : Liu Bei, ayant entendu parler du génie reclus 诸葛亮, vient 3 fois jusqu\'à sa cabane. Aux 2 premières, le sage est absent. À la 3e, Liu Bei attend des heures dans la neige. Ému, Zhuge Liang accepte de devenir son stratège. RÈGLE D\'OR : sens moderne — recruter avec persévérance et humilité. Astuce : phrase 我们要有三顾茅庐的精神 (« il nous faut l\'esprit des 3 visites »). 桃园结义 (« serment du verger de pêchers ») : Liu Bei, Guan Yu et Zhang Fei jurent fraternité de sang dans un verger en fleurs. Règle : modèle de l\'amitié indéfectible. Remarque : si 2 amis chinois disent « 我们桃园结义 », c\'est un compliment fort sur leur lien.',
    bodyEn:
  ...
  },
  {
    ...
    body:
      '望梅止渴 (« voir les prunes pour apaiser sa soif ») : Cao Cao, en marche avec ses troupes assoiffées, leur dit qu\'il y a un verger de prunes derrière la colline. La salivation les fait tenir jusqu\'à la prochaine source. Règle : sens — se consoler d\'un espoir illusoire. Astuce : usage moderne 这只是望梅止渴 = c\'est juste se mentir à soi-même. 草船借箭 (« emprunter des flèches avec des bateaux de paille ») : Zhuge Liang, sommé de produire 100 000 flèches en 3 jours, envoie de nuit des bateaux couverts de paille vers le camp ennemi. Croyant à une attaque, l\'ennemi tire des milliers de flèches qui se fichent dans la paille. RÈGLE D\'OR : sens moderne — ruse géniale qui retourne les ressources de l\'adversaire. Remarque : apprécié des entrepreneurs chinois.',
    bodyEn:
...
export const c12ChengyuAdvancedM2LearnSections: LessonV2LearnSection[] = [
  {
    ...
    body:
      '悬梁刺股 (« suspendre la tresse, piquer la cuisse ») fusionne 2 anecdotes : 孙敬 (Sun Jing, Han) attachait sa tresse à une poutre du plafond pour ne pas s\'endormir en lisant ; 苏秦 (Su Qin, Royaumes combattants) se piquait la cuisse avec une alêne quand il s\'endormait. Devenus tous 2 hauts fonctionnaires. RÈGLE D\'OR : sens — étudier au prix de soi-même. 凿壁偷光 (« percer le mur, voler la lumière ») : 匡衡 (Kuang Heng, Han), trop pauvre pour les bougies, perçait le mur du voisin pour profiter de SA lumière. Devint Premier ministre. Règle : sens — étudier malgré la pauvreté. Remarque : ces 2 chengyu sont sur les murs de toutes les écoles chinoises. Astuce : phrase courante 我们要发扬悬梁刺股的精神 (« il faut perpétuer l\'esprit du sacrifice par l\'étude »).',
    bodyEn:
  ...
  },
  {
    ...
    body:
      '韦编三绝 (« lanières [du livre] rompues 3 fois ») : Confucius, lecteur passionné du 易经 (Yìjīng, Livre des Mutations), aurait lu son exemplaire si souvent que les sangles de cuir reliant les lattes de bambou se sont brisées 3 fois. Règle : sens — lecture assidue et profonde. Astuce : pour un travail intellectuel monumental, 这本书我已经韦编三绝 (j\'ai relu ce livre des dizaines de fois). 学而不厌 (« étudier sans se lasser ») vient des 论语 (Analectes) : 学而不厌，诲人不倦 (étudier sans se lasser, enseigner sans se fatiguer). RÈGLE D\'OR : devise du professeur idéal. Remarque : pour louer un mentor, « 老师真是学而不厌 ». Astuce : combo intellectuel — citer ces 2 chengyu dans une discussion sur l\'éducation = signal C1.2 immédiat.',
    bodyEn:
...
export const c12ChengyuAdvancedM3LearnSections: LessonV2LearnSection[] = [
  {
    ...
    body:
      'RÈGLE D\'OR : un 歇后语 (xiēhòuyǔ) marche en 2 temps : (1) une scène énigmatique, (2) une chute qui dévoile le sens. Règle : souvent, la chute repose sur un jeu phonétique. Astuce : ex emblématique 外甥打灯笼 — 照舅(旧). « Le neveu tient la lanterne : éclaire son oncle (= comme avant) ». Remarque : 照舅 (zhào jiù, éclairer l\'oncle) sonne comme 照旧 (zhào jiù, comme d\'habitude). Règle : à l\'oral, on ne dit souvent que la 1re partie — « 这事啊，外甥打灯笼…» et l\'autre rit en complétant intérieurement. Astuce : si tu peux placer la 1re partie + tes interlocuteurs comprennent, c\'est un moment de complicité culturelle profond. Remarque : apprends 5-10 歇后语 standards pour les déployer.',
    bodyEn:
  ...
  },
  {
    ...
    body:
      '黄鼠狼给鸡拜年 — 没安好心 (« la belette souhaite bonne année au poulet — pas avec de bonnes intentions »). La belette mange les poulets : si elle vient leur souhaiter le Nouvel An, c\'est qu\'elle prépare un coup. Attention : sens — se méfier d\'une amabilité suspecte. 八仙过海 — 各显神通 (« les 8 Immortels traversent la mer — chacun déploie ses talents »). Les 8 Immortels du taoïsme, devant traverser la mer sans bateau, utilisent chacun leur pouvoir magique. RÈGLE D\'OR : sens — que chacun donne le meilleur de soi dans une situation difficile, en mode coopératif. Astuce : à utiliser pour lancer un projet collectif — « 这次项目，我们八仙过海，各显神通 ! » Remarque : très motivant + culturellement riche.',
    bodyEn:
...
export const c12BusinessM1LearnSections: LessonV2LearnSection[] = [
  {
    ...
    body:
      'RÈGLE D\'OR : 关系 (guānxi) ne se déclare pas, il se construit par investissement long-terme. Règle : ressources de base 同学 (camarades de promotion universitaire — relation extrêmement forte en Chine), 同乡 (gens de la même province), 战友 (camarades militaires), 老朋友 (vieux amis). Astuce : mécanique — on s\'invite à manger 吃饭, on s\'offre des cadeaux 送礼 (jamais excessifs sinon malaise), on s\'invite aux mariages et funérailles, on s\'aide concrètement (CV, intro à un médecin, problème admin). RÈGLE D\'OR : la réciprocité est la règle — recevoir sans donner = se ferme. Attention : la pire chose — demander un service à quelqu\'un avec qui tu n\'as jamais 关系, perçu comme grossier. Remarque : construis d\'abord, sollicite ensuite.',
    bodyEn:
  ...
  },
  {
    ...
    body:
      'Règle : 给面子 (gěi miànzi, donner face) = honorer publiquement quelqu\'un. Astuce : accepter une invitation, complimenter en réunion, demander conseil publiquement (montre que tu valorises l\'autre). Règle : 丢面子 (diū miànzi, perdre face) = être humilié publiquement. Attention : critique frontale, contradiction publique, désaccord visible. Remarque : à l\'oral occidental on dit « it\'s ok to disagree » — en chinois, désaccord public = catastrophe relationnelle. RÈGLE D\'OR : 留面子 (liú miànzi, préserver la face de l\'autre) = formule magique. Astuce : si tu vois un collègue se tromper en réunion, ne le corrige pas publiquement — adresse-toi à lui plus tard en privé. Remarque : c\'est 留面子. Attention : ce concept structure 80% des comportements pro chinois — l\'ignorer = sabotage involontaire de toutes tes relations.',
    bodyEn:
...
export const c12BusinessM2LearnSections: LessonV2LearnSection[] = [
  {
    ...
    body:
      'RÈGLE D\'OR : en culture chinoise, manifeste de confiance = manger et boire ensemble. Remarque : au bureau, on est dans la profession ; à table, on est dans l\'humain. Règle : les deals importants se concluent souvent au 2e ou 3e banquet, pas au 1er rendez-vous. Astuce : le 白酒 (alcool de sorgho à 50-55°) joue un rôle clé — il « 拉近距离 » (rapproche les distances). Remarque : plus on a bu ensemble, plus le 关系 est ancré. Attention : pour les buveurs occidentaux, prudence, le 白�el est beaucoup plus fort que le vin. Astuce : stratégie — bois 1 verre quand l\'hôte trinque, mais 意思意思 (petite gorgée symbolique) le reste du temps. Astuce : demande de l\'eau ou du thé entre les toasts. Règle : mange avant de boire — jamais sur estomac vide.',
    bodyEn:
  ...
  },
  {
    ...
    body:
      'Règle : les jeunes générations + secteur tech préfèrent le 商务茶 (shāngwù chá, thé d\'affaires) au 白酒. Remarque : on se retrouve dans une 茶馆 (chá guǎn, maison de thé) ou un salon privé d\'hôtel. Astuce : codes — l\'hôte choisit le thé (普洱 pǔ\'ěr, 龙井 lóngjǐng, 铁观音 tiěguānyīn — 3 thés stars). Préparation par un 茶艺师 (cérémoniel). Remarque : conversation plus calme, plus stratégique. Astuce : avantages — pas d\'ivresse, pas de gueule de bois, on garde la tête claire pour décider. Attention : désavantages — moins de 拉近距离 émotionnel, certains seniors préfèrent encore le 酒桌 traditionnel pour « tester » un partenaire. RÈGLE D\'OR : compromis — démarre la relation au 商务茶, scelle les gros deals au 酒桌.',
    bodyEn:
...
export const c12BusinessM3LearnSections: LessonV2LearnSection[] = [
  {
    ...
    body:
      'RÈGLE D\'OR : en chinois, 打 X 折 = on paie X/10 du prix initial. Règle : donc 打八折 = 80% du prix = remise de 20%. Attention : erreur fréquente du francophone — croire 八折 = 80% de remise. Catastrophe en négociation si tu oublies. Astuce : mnémo — le nombre indique ce que tu paies. Remarque : 打九折 = 90% (remise 10%), 打七折 = 70% (remise 30%), 打半价 = moitié prix. Astuce : pour demander une remise, 能不能打个折 ? (peut-on avoir une remise ?) — formule ouverte. Astuce : pour demander une remise précise, 能打八折吗 ? (80% du prix ?). Remarque : le vendeur peut counter-proposer 打八五折 (85% — moins bonne pour toi). Règle : compétences clés — calculer mentalement la conversion sans se planter.',
    bodyEn:
  ...
  },
  {
    ...
    body:
      'Vocab : 付款方式 (mode de paiement : 现金/转账/支票 cash/virement/chèque), 定金 (acompte, généralement 30%), 尾款 (solde, à la livraison), 分期 (paiement échelonné), 押金 (caution remboursable). Délai : 交货期 (date de livraison), 工期 (durée de chantier). Pénalité : 违约金 (pénalité de rupture, fixée par le contrat). RÈGLE D\'OR : 盖章 (apposer le sceau) — en Chine, le sceau officiel de l\'entreprise (公章 gōngzhāng, sceau rouge rond) prime sur la signature individuelle. Attention : sans sceau, un contrat reste faible même signé. Règle : pour qu\'un document soit légalement contraignant — signature ET sceau. Remarque : les sceaux sont strictement contrôlés par le 财务 (département finance) — leur usage est un acte solennel.',
    bodyEn:
...
export const c12BusinessM4LearnSections: LessonV2LearnSection[] = [
  {
    ...
    body:
      'RÈGLE D\'OR : format obligatoire — (1) Saluer hiérarchiquement (各位领导，各位来宾，大家好 !). (2) Se présenter (我是 X 公司的 Y, 担任 Z 职位). (3) Annoncer le sujet (今天我向各位介绍 X). (4) Énoncer la durée (我的发言大约 X 分钟). (5) Démarrer. Règle : premier paragraphe — présentation entreprise (成立于 X 年, 总部在 X, 主营业务 X). Astuce : pour montrer ta légitimité, 我们在 X 行业拥有 X 年的经验 (nous avons X années d\'expérience dans le secteur X). Remarque : 拥有 (yōngyǒu, posséder) sonne plus pro que 有. Attention : ne jamais commencer par « 大家好 » seul — c\'est trop familier en B2B. Règle : toujours hiérarchiser l\'adresse.',
    bodyEn:
  ...
  },
  {
    ...
    body:
      'RÈGLE D\'OR : le pitch chinois se termine toujours sur une note d\'ouverture et de respect. Règle : vocab — 合作共赢 (coopération gagnant-gagnant), 互利互惠 (mutuellement bénéfique), 携手共进 (avancer main dans la main). Astuce : phrase 期待与贵公司携手合作，共创美好未来 (j\'attends notre collaboration main dans la main pour créer un bel avenir). Remarque : 共创 (cocréer) + 美好未来 (bel avenir) = combo très chinois. Astuce : chengyu de fin — 互利共赢 ou 共创未来 ou 蒸蒸日上 (en plein essor). RÈGLE D\'OR : le 贵 (gùi, votre honorable) précédant 公司/校/国 = marque de respect obligatoire pour parler de l\'entité du partenaire. Attention : sans 贵, ton pitch sonne banal.',
    bodyEn:
...
export const c12EducationSystemM1LearnSections: LessonV2LearnSection[] = [
  {
    ...
    body:
      'Tableau type d\'un Chinois né en 2010 : 2013-16 maternelle (3 ans), 2016-22 primaire (6 ans, examens internes), 2022-25 collège (3 ans, fin par 中考), 2025-28 lycée (3 ans, fin par 高考), 2028-32 licence (4 ans), peut-être 2032-35 master, 2035-40 doctorat. Règle : 义务教育 (scolarité obligatoire) couvre 9 ans (primaire + collège), gratuite. Remarque : lycée et fac payants (modeste pour le public, cher pour le privé). Astuce : heures de cours — enfant chinois moyen = 8h-17h école + 19h-22h devoirs/cours particuliers. Attention : phénomène 鸡娃 (jīwá, « enfant-poule ») — parents qui sur-poussent leurs enfants académiquement. Remarque : sujet de société majeur — burnout, suicide d\'adolescents, pression psychologique.',
    bodyEn:
  ...
  },
  {
    ...
    body:
      'Règle : le 课外辅导 (kèwài fǔdǎo, cours particuliers extra-scolaires) était une industrie de 100+ milliards $ en 2020 (新东方, 学而思 leaders). RÈGLE D\'OR : en 2021, politique « 双减 » (shuāng jiǎn, « double réduction ») — réduction des devoirs + interdiction des cours particuliers à but lucratif pour les matières du curriculum (math, anglais). Attention : effets — faillite massive des entreprises, chômage de profs, mais cours souvent reportés en clandestin (1-on-1 à domicile). Remarque : vocab — 补习班 (cours de rattrapage), 一对一 (yī duì yī, cours particulier 1-on-1), 在线教育 (cours en ligne), 学区房 (xuéqūfáng, appartement situé dans le secteur d\'une bonne école — flambée des prix). Attention : le sujet est politique-explosif — il révèle l\'inégalité d\'accès à l\'éducation.',
    bodyEn:
...
export const c12EducationSystemM2LearnSections: LessonV2LearnSection[] = [
  {
    ...
    body:
      'Règle : le 高考 a lieu chaque année le 7-9 juin. Remarque : plus de 10 millions de candidats simultanément. RÈGLE D\'OR : mobilisation nationale — circulation interdite autour des centres d\'examen, parents en attente toute la journée, chauffeurs de taxi gratuits pour les candidats, police présente. Astuce : tradition — la mère porte un 旗袍 (qípáo, robe traditionnelle) — 旗 sonne comme « ouverture / succès ». Remarque : la famille mange un bol de 鱼 (poisson — homophone 余 « surplus ») la veille. Astuce : le candidat tient un porte-bonheur (jade, médaille de Confucius). Remarque : c\'est un événement national qui suspend le pays. Règle : vocab — 考场 (centre d\'examen), 监考 (surveillant), 答题卡 (feuille de réponses), 准考证 (carte d\'admission). Astuce : si tu es en Chine début juin, regarde un JT — la couverture du 高考 est intense.',
    bodyEn:
  ...
  },
  {
    ...
    body:
      'Règle : 985 工程 (lancé en 1998 — donc 9-8) — 39 universités d\'élite, financement massif. Remarque : 清华 (Tsinghua), 北大 (Pékin), 复旦, 上海交大, 浙大, 中科大 (USTC). Astuce : diplômés très recherchés en entreprise. Règle : 211 工程 (lancé pour le 21e siècle — top 100 universités) — groupe plus large incluant les 985 + universités provinciales fortes. RÈGLE D\'OR : sortir d\'une 985 ou 211 = avantage carrière énorme. Attention : la discrimination implicite à l\'embauche existe — certains employeurs ne lisent même pas un CV non-985/211. Remarque : récemment, le programme « 双一流 » (Double First-Class) remplace officiellement 985/211 mais les anciens labels restent dans le langage. Astuce : hors 985/211 — 普通本科 (université régulière), 专科 (cycle court 2-3 ans, équivalent BTS).',
    bodyEn:
...
export const c12EducationSystemM3LearnSections: LessonV2LearnSection[] = [
  {
    ...
    body:
      'Règle : 海归 (hǎiguī, « tortue de mer ») = jeu de mots avec 海归 (« revenu d\'outre-mer »). Remarque : désigne un Chinois qui a étudié à l\'étranger et est rentré. Astuce : dans les années 2000-2010, statut prestigieux, salaires majorés, postes de cadre direct. Attention : aujourd\'hui (2020s), ambivalence. Remarque : la Chine forme excellemment ses élites, les universités locales rivalisent avec les Ivy League. Attention : de plus, la pandémie + tensions géopolitiques + xénophobie post-2020 ont déprécié certains diplômes étrangers. Astuce : nouveau terme — 海带 (hǎidài, « algue marine » homophone de 海待 « 海归 en attente ») = 海归 au chômage, narratif d\'ironie. RÈGLE D\'OR : la valeur d\'un 海归 dépend désormais de l\'université + du domaine. Remarque : 海归 d\'Oxford en physique = très valorisé ; 海归 d\'une école obscure en marketing = doute.',
    bodyEn:
  ...
  },
  {
    ...
    body:
      'Règle : côté entrants étrangers — 中国政府奖学金 (CSC, China Scholarship Council, équivalent du programme Erasmus+ pour étudier en Chine) couvre frais de scolarité + logement + assurance + allocation mensuelle (~3000-3500 ¥). Astuce : postuler via l\'ambassade de Chine ou l\'université d\'accueil. Règle : côté Chinois sortants — 国家公派留学 (financement État pour étudier à l\'étranger), souvent doctorat avec engagement de retour. Remarque : échanges — 交换生 (semestre/an dans une université partenaire), 双学位 (double diplôme). RÈGLE D\'OR : pour étudier dans une 985 (Tsinghua, Pékin) gratuitement, le CSC est ta meilleure porte d\'entrée. Astuce : process compétitif mais accessible si dossier solide. Attention : deadline typique — décembre-mars pour rentrée septembre suivante.',
    bodyEn:
...
export const c12LawSocietyM1LearnSections: LessonV2LearnSection[] = [
  {
    ...
    body:
      'Règle : au sommet — 宪法 (xiànfǎ, Constitution, adoptée 1982, amendée 5 fois — la plus récente en 2018 pour autoriser un 3e mandat de Xi Jinping). Règle : sous — 法律 (lois adoptées par 全国人民代表大会, l\'ANP — Assemblée nationale populaire, ou par son Comité permanent). Astuce : encore en dessous — 行政法规 (règlements du Conseil d\'État), 部门规章 (arrêtés ministériels), 地方性法规 (règlements locaux). RÈGLE D\'OR : spécificité — le droit chinois est civiliste (codifié, pas common law). Attention : les arrêts judiciaires n\'ont pas valeur de précédent. Remarque : code civil unifié promulgué en 2020 (《民法典》, le 1er code civil de l\'histoire de la Chine populaire). RÈGLE D\'OR : pour un litige international, exige dans tes contrats — juridiction + langue + droit applicable spécifiés.',
    bodyEn:
  ...
  },
  {
    ...
    body:
      'Règle : côté demandeur — 原告 (plaignant) dépose 起诉 (poursuite). Règle : côté défendeur — 被告 (accusé/défendeur) reçoit 传票 (assignation). Astuce : représentation — chacun a 律师 (avocat). Remarque : le 法官 (juge) instruit. Règle : la procédure passe par 一审 (1re instance, tribunal de base ou intermédiaire), si on n\'est pas content — 上诉 (appel) → 二审 (2e instance). RÈGLE D\'OR : en Chine, pas de jury populaire — uniquement des juges professionnels. Attention : audience publique sauf cas sensibles (mineurs, sécurité d\'État, vie privée). Remarque : issue — 判决 (jugement) ou 调解 (médiation, encouragée par le système). Astuce : peines pénales — 罚款 (amende), 有期徒刑 (prison à durée déterminée), 无期 (perpétuité), 死刑 (peine de mort, encore appliquée). Remarque : procès chinois en moyenne plus rapide qu\'en Occident.',
    bodyEn:
...
export const c12LawSocietyM2LearnSections: LessonV2LearnSection[] = [
  {
    ...
    body:
      'Règle : en Chine, le 女权主义 (féminisme) reste un mouvement contesté, parfois censuré sur les réseaux sociaux (Weibo a fermé plusieurs comptes féministes en 2021-22). Attention : le terme 剩女 (« femmes restantes ») a été propagé par l\'État (campagne de l\'agence All-China Women\'s Federation) pour pousser les femmes 27+ à se marier vite. Remarque : vu critiquement comme outil de contrôle social. Astuce : mouvement #MeToo chinois (2018) — explosion brève, puis répression. Remarque : cas emblématique — Peng Shuai (joueuse de tennis, 2021, accusations contre un ex-vice-Premier ministre — disparition publique). Attention : discours pour les femmes — pression simultanée à se marier, faire des enfants, et réussir sa carrière. Remarque : phénomène 全职妈妈 (mère au foyer) ré-émerge sous l\'effet de la politique « 3 enfants » + difficultés des crèches.',
    bodyEn:
  ...
  },
  {
    ...
    body:
      'Règle : politique 计划生育 (planification familiale) — 一孩 (1 enfant) imposé en 1978 sous Deng Xiaoping pour freiner la croissance. Attention : conséquences — 30M+ avortements forcés/stérilisations, déséquilibre filles/garçons (manque de 30M de femmes), génération 独生子女 (enfants uniques). Remarque : 2016, 二孩 (2 enfants autorisés). Remarque : 2021, 三孩 (3 enfants, en réaction à l\'effondrement de la natalité). Attention : effet minime. RÈGLE D\'OR : la société urbaine a intégré la culture de l\'enfant unique + coût élevé du logement, éducation, garderie = peu d\'envie. Remarque : 2022, pour la 1re fois depuis 1961, la population chinoise a diminué. Astuce : Inde dépasse la Chine en population en 2023. Attention : vieillissement accéléré — défi majeur du 21e siècle pour la Chine.',
    bodyEn:
...
export const c12LawSocietyM3LearnSections: LessonV2LearnSection[] = [
  {
    ...
    body:
      'Règle : 防火长城 (fánghuǒ chángchéng, « grande muraille de feu ») = surnom officieux du Great Firewall (GFW). Remarque : officiel — 网络长城. RÈGLE D\'OR : bloque l\'accès aux services occidentaux (Google, FB, Twitter/X, YouTube, Instagram, WhatsApp, Wikipedia partiel, NYT, BBC Chinese, etc.). Astuce : substituts chinois — 百度 (Baidu) pour Google, 微信 (WeChat) pour WhatsApp, 微博 (Weibo) pour Twitter, 抖音 (Douyin) pour TikTok, 哔哩哔哩 (Bilibili) pour YouTube. Règle : contournement — 翻墙 (fān qiáng, « sauter le mur » = utiliser un VPN). Attention : légalement zone grise — toléré pour étrangers + chercheurs, plus risqué pour citoyens chinois. Remarque : en 2026, le GFW est plus efficace qu\'en 2015 — beaucoup de VPN bloqués. Astuce : si tu vis en Chine, prépare un VPN d\'entreprise fiable avant ton arrivée.',
    bodyEn:
  ...
  },
  {
    ...
    body:
      'RÈGLE D\'OR : 网络安全法 (loi sur la cybersécurité, 2017) impose — (1) hébergement local des données personnelles des Chinois, (2) coopération avec les autorités, (3) certification des équipements critiques. Règle : loi 2021 — 数据安全法 (loi sur la sécurité des données) + 个人信息保护法 (PIPL — équivalent du RGPD européen, mais plus strict côté étatique). Attention : pour une entreprise étrangère opérant en Chine — haut coût de conformité (audit, hébergement, transferts encadrés). Astuce : censure de contenu via 敏感词 (mots-clés filtrés automatiquement) — 64 (= 4 juin 1989, Tiananmen), noms de dissidents (刘晓波, 艾未未), références au Tibet/Xinjiang/Falun Gong. Remarque : sur WeChat, Weibo, etc., un message contenant un 敏感词 peut être supprimé silencieusement. Attention : évite ces sujets en ligne en Chine — règle de prudence basique.',
    bodyEn:

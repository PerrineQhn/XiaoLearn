...
export const b21GrammarLianM1LearnSections: LessonV2LearnSection[] = [
  {
    ...
    body:
      'Choisis un nom qui surprend : le plus petit, le plus évident, le plus improbable. Place-le entre 连 et 也/都. L\'effet ? L\'interlocuteur comprend que TOUT le reste est concerné a fortiori. 连小孩也知道 = même un enfant sait → donc tout adulte le sait aussi. Sans 连, tu dis un fait ; avec 连, tu fais un argument.',
    bodyEn:
...
export const b21GrammarLianM2LearnSections: LessonV2LearnSection[] = [
  {
    ...
    body:
      'Structure : S + 连 + V + 也/都 + 不/没 + V (+ complément). Le verbe apparaît deux fois : la première après 连 ressemble à une affirmation, la seconde porte la vraie négation. En français : « même pas + V ». 他连看也没看一眼 = il n\'a même pas jeté un regard. Très oral, très expressif — parfait pour montrer la surprise ou l\'indignation.',
    bodyEn:
...
export const b21GrammarLianM3LearnSections: LessonV2LearnSection[] = [
  {
    ...
    body:
      'Tout dépend de la SECONDE partie. Avec 还/也 → inclusion : « à part X, il y a AUSSI Y » (les deux). Avec 都 + négation → exclusion : « à part X, tous » (X est hors). 除了中文以外，我还会英文 → je parle les deux. 除了小王以外，大家都来了 → Xiao Wang est le seul absent. Un changement d\'auxiliaire renverse le sens à 180 degrés.',
    bodyEn:
  ...
  },
  {
    ...
    body:
      'Trois cousins à ne pas confondre. **除了…以外** = inclusion / exclusion. **除非 A，否则 B** = « à moins que A, sinon B » — condition exclusive, très utilisée pour les menaces ou ultimatums : 除非你道歉，否则我不去 = à moins que tu ne t\'excuses, je n\'y vais pas. **除…之外** = variante écrite/soutenue de 除…以外, préférée dans la presse : 除个别案例之外，方法可行 = hormis quelques cas isolés, la méthode est viable.',
    bodyEn:
...
export const b21GrammarConjM1LearnSections: LessonV2LearnSection[] = [
  {
    ...
    body:
      'La formule classique pour enchaîner deux qualités / deux actions. Même sujet : 他不但聪明，而且努力. Sujets différents : le second sujet va APRÈS 而且 — 不但他来，而且他弟弟也来. Pour plus formel, remplace 不但 par 不仅 (écrit). 而且 peut se remplacer par 并且, 也, 还. Évite d\'empiler 也 ET 而且 dans la même partie.',
    bodyEn:
  ...
  },
  {
    ...
    body:
      'Pour une progression qui surprend, remplace 而且 par 甚至 (« voire, au point que »). 他不但聪明，甚至可以用十种语言写作 = non seulement il est intelligent, il écrit même dans dix langues. 甚至 introduit un élément que l\'interlocuteur n\'attendait pas — on dépasse l\'addition simple pour entrer dans le spectaculaire. Combinaison favorite à l\'écrit : 不仅…甚至. Cette escalade structure bien les plaidoiries, critiques d\'art et éditoriaux.',
    bodyEn:
...
export const b21GrammarConjM2LearnSections: LessonV2LearnSection[] = [
  {
    ...
    body:
      'Pour dire « peu importe X, Y », il TE FAUT : (1) 无论 (écrit) ou 不管 (oral), (2) un mot interrogatif 谁/什么/哪/怎么 OU une alternative A还是B, (3) 都 ou 也 dans la seconde clause. Sans ces trois pièces, la structure est bancale. 无论谁来，我都欢迎. 不管多忙，他都去健身房.',
    bodyEn:
...
export const b21GrammarConjM3LearnSections: LessonV2LearnSection[] = [
  {
    ...
    body:
      '即使 introduit un cas imaginé, souvent extrême : « même si X arrivait, B quand même ». 即使下雨，我也去 = même s\'il pleut, j\'y vais. 虽然 au contraire constate un fait réel : 虽然下雨了，我还是去了 = il a plu (réel) et je suis quand même allé. Pour forcer le cas extrême, prends 哪怕 : 哪怕只剩一块钱，我也要走.',
    bodyEn:
  ...
  },
  {
    ...
    body:
      'Un mouvement fréquent à l\'écrit HSK 5/6 : d\'abord concéder un fait (虽然 + réel), puis pousser l\'hypothèse extrême (即使 + imaginé). Ex : 虽然这次失败了，但即使下次还失败，我也不会放弃 = j\'ai échoué cette fois, mais même si j\'échoue encore la prochaine, je n\'abandonnerai pas. Le 也 est OBLIGATOIRE dans la clause 即使 (jamais 就). Si tu l\'oublies, la phrase s\'effondre — c\'est la faute la plus corrigée par les profs chinois.',
    bodyEn:
...
export const b21GrammarConjM4LearnSections: LessonV2LearnSection[] = [
  {
    ...
    body:
      '虽然 et 尽管 = « bien que », mais 尽管 ajoute une nuance d\'effort ou d\'obstacle surmonté. 虽然很累，但是我继续工作 = je continue malgré la fatigue (neutre). 尽管很累，他还是坚持到最后 = bien qu\'épuisé, il a tenu jusqu\'au bout (on admire l\'effort). Piège majeur : 尽管 seul, sans seconde proposition, signifie « n\'hésitez pas » — 你尽管说 = parlez sans retenue.',
    bodyEn:
  ...
  },
  {
    ...
    body:
      'Hors structure concessive, 尽管 devant un verbe signifie « sans retenue, sans hésiter ». 你尽管说 = parle sans retenue. 有什么问题，你尽管问 = pose toutes tes questions, sans gêne. 你尽管试 = essaie librement. Ce sens est ultra-fréquent à l\'oral et c\'est un faux-ami redoutable : des étudiants traduisent mécaniquement par « bien que » et produisent du charabia. Contextualise TOUJOURS avant de traduire.',
    bodyEn:
...
export const b21ConvM1LearnSections: LessonV2LearnSection[] = [
  {
    ...
    body:
      'Ouvrir : 大家好，我们今天的会议有三个议题 (bonjour, on a 3 sujets à l\'agenda). Lancer un sujet : 第一个议题是 X / 我们先来讨论 X. Donner la parole : 老张，你先说 / 你怎么看 ? Recadrer : 这个问题我们一会儿再讨论 / 我们先回到主题. Vocab-clé : 议题 (yìtí, ordre du jour), 主持 (zhǔchí, présider), 主题 (sujet), 进度 (avancement). Phrase culturellement attendue : 大家有什么想法都可以提 (n\'hésitez pas à proposer vos idées) — montre que tu invites la participation, sinon les Chinois plus jeunes attendent que tu désignes.',
    bodyEn:
...
export const b21ConvM2LearnSections: LessonV2LearnSection[] = [
  {
    ...
    body:
      'Ouvrir : 我们想了解一下贵公司的报价 (on aimerait connaître votre offre). Pousser : 这个价格我们觉得有点高 (un peu élevé), 还有没有调整的空间 ? (y a-t-il de la marge ?). 空间 (espace) = vocabulaire de négociation chinois clé. Vous proposer un compromis : 如果 X，我们可以考虑 Y. Forcer la main poliment : 我们的预算有限 (notre budget est limité), 这是我们的最后报价 (c\'est notre dernière offre). Conclure : 我们达成协议了 ! (on a un accord !). En culture chinoise, négocier dur est NORMAL et attendu — accepter le 1er prix peut faire perdre du respect.',
    bodyEn:
...
export const b21ConvM3LearnSections: LessonV2LearnSection[] = [
  {
    ...
    body:
      'Structure : 项目目前的进度 (l\'avancement actuel), 已完成的部分 (ce qui est fait), 正在进行 (en cours), 计划中的下一步 (étapes prévues). Vocab : 完成度 (taux d\'achèvement), 进展顺利 (avancée fluide), 遇到了一些问题 (rencontré des difficultés). Phrase positive : 总体上按计划进行 (globalement dans les temps). Si retard : 因为 X，我们可能要延期一周 (cause + délai). Toujours proposer une SOLUTION en même temps : 我们的解决方案是 Y. Reporter un problème SANS solution est culturellement perçu comme manque d\'initiative.',
    bodyEn:
  ...
  },
  {
    ...
    body:
      'Introduire : 根据我们的数据 (selon nos données). Verbes : 增长 (croissance), 下降 (baisse), 保持 (maintenir), 超过 (dépasser), 达到 (atteindre). Quantifier : 增长了 X 个百分点 = augmentation de X points (≠ X% — différence subtile mais importante). Comparer : 比上个季度 X (vs trimestre précédent). Souligner : 值得注意的是 X (ce qui est notable c\'est X). Conclure : 总体来看 (globalement), 数据显示 X (les données montrent X). Phrase nuancée : 这个数据需要进一步分析 (ces données méritent plus d\'analyse) — montre la prudence intellectuelle, valorisée en pro chinois.',
    bodyEn:
...
export const b21ConvM4LearnSections: LessonV2LearnSection[] = [
  {
    ...
    body:
      'Reconnaître la tension : 我感觉我们之间有一些误会 (j\'ai l\'impression qu\'il y a un malentendu). Inviter à la discussion : 我们能不能坐下来好好聊一下 ? (peut-on s\'asseoir pour en discuter ?). 误会 (wùhuì, malentendu) est le mot-clé : il EXTÉRIORISE la cause sans accuser. Préfère TOUJOURS « il y a un malentendu » à « tu as fait une erreur ». Pour exposer ton point : 从我的角度看 X, 让我们澄清一下 (clarifions). Pour reconnaître ta part : 我也有责任 (j\'ai aussi ma part). En culture chinoise, partager la responsabilité 50/50 conclut souvent un conflit même quand l\'autre a 80% de torts.',
    bodyEn:
  ...
  },
  {
    ...
    body:
      'Position neutre : 我不想偏袒任何一方 (je ne veux pas prendre parti). 偏袒 (piāntǎn, favoriser) est un mot lourd, à utiliser pour signaler ta neutralité. Inviter chacun à exprimer : 你先说，我听 / 现在轮到你 (à toi maintenant). Synthétiser : 我听到的是 X，对吗 ? (ce que j\'entends c\'est X, correct ?). Proposer un terrain commun : 我们能不能找一个双方都能接受的方案 ? (on peut chercher une solution acceptable pour les deux ?). 双方 (les deux parties) est le mot-clé. Conclure : 这件事就到这里 (l\'affaire s\'arrête là) — invite chacun à passer à autre chose.',
    bodyEn:
...
export const b21ConvM5LearnSections: LessonV2LearnSection[] = [
  {
    ...
    body:
      'Aborder : 不好意思，可以加您一下微信吗 ? (excusez, je peux vous ajouter sur WeChat ?). Le QR code WeChat = la carte de visite chinoise moderne. Présenter : 我是 X 公司的 X，我做 X (je suis X de l\'entreprise X, je travaille en X). Trouver un point commun : 我们都做 X (on fait tous les deux X). Pour l\'après-rencontre : 很高兴认识您 (ravi de faire votre connaissance), 以后多联系 (on garde contact). Sur WeChat juste après : 您好，我是今天活动认识的 X (bonjour, je suis X rencontré à l\'événement). Sans ce message dans les 24 h, ton ajout est oublié.',
    bodyEn:
  ...
  },
  {
    ...
    body:
      'Wishes saisonniers : 中秋节快乐 / 春节快乐 (envoyés à TOUTES les relations pro chinoises ; absence = note négative). Anniversaire : noter le 生日 visible sur WeChat et envoyer 生日快乐 + emoji. Échanges utiles : 看到这篇文章，想到你 + lien (j\'ai vu cet article, j\'ai pensé à toi). Demander un service après long silence : 不好意思打扰您，最近有件事想麻烦您 + précision. Phrase qui sauve : 好久没联系，希望您一切都好 (longtemps sans contact, j\'espère que tout va bien). Réinitialise une relation pro endormie poliment.',
    bodyEn:
...
export const b21ConvM6LearnSections: LessonV2LearnSection[] = [
  {
    ...
    body:
      'Ouvrir : 各位领导，各位同事，下午好 (mesdames et messieurs les responsables, chers collègues, bonjour). Le hello 各位 (chacun de vous) + hiérarchie est culturellement attendu. Reconnaître l\'audience : 非常感谢大家今天能抽时间参加 (merci d\'avoir pris le temps de venir). Annoncer le sujet : 今天我想跟大家分享 X. Clôturer : 我的发言到此结束 (mon discours s\'achève ici), 谢谢大家的聆听 (merci de votre écoute attentive). Phrase de conclusion typique : 希望今天的内容对大家有所帮助 (j\'espère que ce contenu vous a été utile). Très formel, très chinois.',
    bodyEn:
  ...
  },
  {
    ...
    body:
      'Levée de verre : 我提议大家举杯 (je propose qu\'on lève nos verres). Dédier : 为我们的合作干杯 ! (à notre collaboration, santé !). 干杯 (gānbēi, litt. « assécher la coupe ») = cul sec dans les vraies cérémonies, mais aujourd\'hui on tolère « 随意 » (suíyì, à votre convenance — petite gorgée). Phrases-clés : 祝大家身体健康，工作顺利 (santé et succès professionnel à tous). Pour un succès récent : 庆祝我们项目的成功 ! Ne JAMAIS commencer à boire avant le toast. La séquence chinoise = leader propose → tous boivent → on peut boire ensuite. Brûler l\'étape = manque de respect.',
    bodyEn:
...
export const b21ConvM7LearnSections: LessonV2LearnSection[] = [
  {
    ...
    body:
      'Ouverture pro : 尊敬的招聘负责人 (cher responsable de recrutement) — formule consacrée, plus formelle que 你好. Présentation : 我是 X，毕业于 X 大学，目前在 X 公司工作. Motivation : 我对贵公司的 X 项目非常感兴趣 (très intéressé par le projet X). Compétences : 我具备 X / 我有 X 经验. Conclusion : 期待您的回复 (j\'attends votre réponse) puis 此致 + 敬礼 (formule de clôture obligatoire en chinois — l\'équivalent du « cordialement »). Sans 此致敬礼, ta lettre paraît incomplète/irrespectueuse en milieu pro chinois.',
    bodyEn:
  ...
  },
  {
    ...
    body:
      'Sujet (主题) : court et clair (X 项目的进度更新). Ouverture : 您好，X (poli) ou 尊敬的 X 先生/女士 (très formel). Corps : courtoisie + objet + détail + action attendue. Relance : 不知道您是否收到了上次的邮件 (je ne sais pas si vous avez reçu mon dernier mail) — non accusateur. Demande : 麻烦您 + verbe (« merci de bien vouloir »). Clôture FORMELLE : 此致 / 敬礼 ou 顺祝商祺 (vœux de prospérité commerciale — très chic). Signature : nom + titre + entreprise. Phrase magique pour conclure poliment : 期待您的回复 (j\'attends votre retour avec intérêt).',
    bodyEn:
...
export const b21NuancesM1LearnSections: LessonV2LearnSection[] = [
  {
    ...
    body:
      '反正 (fǎnzheng, de toute façon, oral familier) = peu importe ce qui s\'est passé, voici la conclusion. 反正我不去 = de toute façon je n\'y vais pas. 无论如何 (wúlùn rúhé, en tout état de cause, formel) = quelles que soient les circonstances. 无论如何，我们必须完成 = en tout état de cause, on doit finir. 不管 (bùguǎn, peu importe, neutre, oral et écrit) = peu importe X, Y reste vrai. 不管下不下雨，我都去 = qu\'il pleuve ou non, j\'y vais. Hierarchy : 反正 (oral, attitude « peu importe je tranche ») < 不管 (neutre conditionnel) < 无论如何 (formel, engagement absolu). 反正 peut sonner fataliste/désinvolte ; 无论如何 montre la détermination.',
    bodyEn:
  ...
  },
  {
    ...
    body:
      '所以 (suǒyǐ, donc — NEUTRE oral et écrit). Le plus universel. 因此 (yīncǐ, par conséquent — un peu plus écrit). Sons sérieux, structurant. 因而 (yīn\'ér, de ce fait — FORMEL écrit). 故 (gù, ainsi — TRÈS FORMEL, écrit académique/juridique/littéraire, parfois poétique). Hierarchy : 所以 (universel) < 因此 (mixte) < 因而 (écrit) < 故 (très formel). À l\'oral, 所以 partout. À l\'écrit pro, alterne 因此 et 因而. JAMAIS 故 à l\'oral spontané. Combo classique en démonstration : 由于 X，故 Y (étant donné X, donc Y) — très juridique.',
    bodyEn:
...
export const b21NuancesM2LearnSections: LessonV2LearnSection[] = [
  {
    ...
    body:
      '关于 (guānyú, concernant) introduit le SUJET d\'une discussion. 关于这个问题，我有几点想法 = concernant cette question, j\'ai quelques idées. Sert à OUVRIR un sujet. 对于 (duìyú, à l\'égard de, vis-à-vis de) introduit la POSITION sur un sujet. 对于这个问题，我的态度是… = vis-à-vis de cette question, ma position est… Plus engageant. 至于 (zhìyú, quant à) introduit un CHANGEMENT de sujet/aspect. 这是大原则。至于细节，我们以后讨论 = c\'est le grand principe. Quant aux détails, on en discute plus tard. 至于 marque une transition organisée. Erreur classique : confondre 关于 (sujet) et 对于 (position).',
    bodyEn:
...
export const b21NuancesM4LearnSections: LessonV2LearnSection[] = [
  ...
  },
  {
    ...
    body:
      '不但 (búdàn) = non seulement (NEUTRE oral et écrit, le plus universel). 不但 X 而且 Y. 不只 (bùzhǐ) = pas seulement (ORAL, plus familier). 不只我，他也喜欢 = pas seulement moi, lui aussi. 不仅 (bùjǐn) = non seulement (FORMEL écrit, plus structurant). 不仅 X 而且 Y. Hierarchy : 不只 (oral) < 不但 (neutre) < 不仅 (écrit) < 不仅仅 (insistant). À l\'oral, 不但 ou 不只. À l\'écrit B2.1+, alterne 不仅 et 不但 pour le rythme. Combo formel intensif : 不仅仅是 X 的问题 (ce n\'est pas QU\'une question de X) — formule rhétorique très efficace.',
    bodyEn:
...
export const b21NuancesM5LearnSections: LessonV2LearnSection[] = [
  {
    ...
    body:
      '居然 (jūrán, figure-toi que / contre toute attente) = surprise + souvent NÉGATIVE/incompréhension. 他居然忘了我的生日 = il a osé oublier mon anniversaire (incompréhensible). 竟然 (jìngrán, identique mais ORAL plus expressif). 你竟然来了 ! = mais tu es venu ! 偏偏 (piānpiān, justement / précisément, comme un fait exprès) = NÉGATIF surtout, marque la malchance ou la persistance. 偏偏今天下雨 = il a fallu qu\'il pleuve juste aujourd\'hui. 我说东，他偏偏要西 = je dis est, il insiste sur ouest. Hierarchy : 居然 (surprise écrite/oral) ≈ 竟然 (oral expressif) ≠ 偏偏 (malchance/contrariété).',
    bodyEn:
...
export const b21NuancesM6LearnSections: LessonV2LearnSection[] = [
  {
    ...
    body:
      '何况 (hékuàng, d\'autant plus que / sans parler de — formel, RENFORCE l\'argument). 这件事连专家都不知道，何况我 ? = même les experts ne savent pas, alors moi ? Marque le fortiori. 况且 (kuàngqiě, en plus, formel écrit) = ajoute un argument supplémentaire dans une démonstration. 这个方案有风险，况且费用高 = ce plan est risqué, et en plus il coûte cher. 再说 (zàishuō, d\'ailleurs, oral) = ajoute un argument supplémentaire moins formel. 我不喜欢这部电影，再说太长了 = je n\'aime pas ce film, et puis il est trop long. Hierarchy : 再说 (oral) < 况且 (formel écrit) < 何况 (à fortiori, percutant).',
    bodyEn:
  ...
  },
  {
    ...
    body:
      '另外 (lìngwài, par ailleurs / en outre — neutre, oral et écrit). 我喜欢这个，另外那个也不错 = j\'aime celui-là, et par ailleurs l\'autre est bien aussi. Polyvalent. 此外 (cǐwài, en outre — formel, écrit). 这个项目有几个特点。此外，它还有 X 个优势 = ce projet a plusieurs caractéristiques. En outre, il a X avantages. Plus structuré. À l\'oral, JAMAIS 此外 (sonne pédant). À l\'écrit B2.1+, 此外 est l\'outil n°1 pour ENCHAÎNER les arguments dans un essai/rapport. Combo classique : 首先 X，其次 Y，此外 Z (premièrement X, deuxièmement Y, en outre Z) — structure d\'essai par excellence.',
    bodyEn:
...
export const b21NuancesM7LearnSections: LessonV2LearnSection[] = [
  ...
  },
  {
    ...
    body:
      '越来越 + adjectif/verbe = de plus en plus. 天气越来越冷 = il fait de plus en plus froid. Marque une PROGRESSION continue. La structure la plus courante. 越 X 越 Y = plus X, plus Y (DEUX variables qui augmentent ensemble). 越说越生气 = plus on en parle, plus on s\'énerve. Différence : 越来越 = UNE variable progresse ; 越…越 = relation entre DEUX variables. Erreur classique : 越来越说越来越生气 ✗ → 越说越生气 ✓. Variantes formelles : 日益 (rìyì, de jour en jour, formel écrit). 经济日益发展 = l\'économie se développe de jour en jour. 与日俱增 (chengyu : croître chaque jour).',
    bodyEn:

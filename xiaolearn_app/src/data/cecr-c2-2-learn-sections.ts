...
export const c22NuancesM2LearnSections: LessonV2LearnSection[] = [
  ...
  },
  {
    ...
    body:
      'RÈGLE D\'OR : les chengyu avec NOMBRES sont des marqueurs lettrés essentiels. Règle : 一鼓作气 (yī gǔ zuò qì) = avancer en un seul élan, sans relâche. À utiliser pour exhorter à finir une tâche d\'un coup. Règle : 三思而行 (sān sī ér xíng) = réfléchir 3 fois avant d\'agir. À utiliser pour conseiller la prudence. Règle : 五湖四海 (wǔ hú sì hǎi) = des 5 lacs et 4 mers (= du monde entier). 来自五湖四海 = venant du monde entier. Règle : 九牛二虎之力 (jiǔ niú èr hǔ zhī lì) = la force de 9 bœufs et 2 tigres = effort surhumain. 用了九牛二虎之力才完成 = il a fallu un effort surhumain pour finir. Astuce : ces 4 chengyu numérotés couvrent 80% des situations émotives du quotidien soutenu chinois.',
    bodyEn:
...
export const c22NuancesM7LearnSections: LessonV2LearnSection[] = [
  {
    ...
    body:
      'RÈGLE D\'OR : chengyu pour comparer / lier / opposer. Règle : 形影不离 (xíng yǐng bù lí) = forme et ombre inséparables = être TOUJOURS ENSEMBLE (couple, amis fusionnels). 这两兄弟形影不离 (ces 2 frères sont inséparables). Règle : 唇齿相依 (chún chǐ xiāng yī) = lèvres et dents s\'entraident = INTERDÉPENDANCE PROFONDE (souvent 2 pays alliés). 中朝两国唇齿相依 (Chine et Corée du Nord sont interdépendantes — usage géopolitique classique). Règle : 千差万别 (qiān chā wàn bié) = mille différences, dix mille distinctions = TOUT DIFFÉRENT, immense diversité. 各人的看法千差万别 (les avis des gens sont infiniment divers). Astuce : ces chengyu de comparaison ENRICHISSENT l\'oral et l\'écrit C2.2 sans pédanterie.',
    bodyEn:
  ...
  },
  {
    ...
    body:
      'RÈGLE D\'OR : le chinois soutenu adore les RÉDUPLICATIONS qui créent un rythme et une musicalité. Règle : 莹莹 (yíng yíng) = brillant brillant (yeux qui scintillent). 泪光莹莹 (les larmes scintillent). Règle : 渐渐 (jiànjiàn) = peu à peu, graduellement. 天渐渐亮了 (le jour se lève peu à peu). Règle : 缓缓 (huǎn huǎn) = lentement, calmement. 缓缓地走 (marcher lentement). Règle : 默默 (mò mò) = silencieusement, discrètement. 默默地工作 (travailler en silence). Astuce : ces 4 redoublements + une centaine d\'autres (悠悠 paisiblement, 漫漫 longuement, 茫茫 immense) construisent l\'ATMOSPHÈRE poétique chinoise. Remarque : maîtriser 5-10 redoublements à l\'oral C2.2 = signal de NIVEAU LITTÉRAIRE et émotionnel maximal. Référence : 李清照 ouvre 《声声慢》 par 7 redoublements consécutifs.',
    bodyEn:

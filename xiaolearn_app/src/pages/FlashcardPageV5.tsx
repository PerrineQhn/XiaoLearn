...
export default function FlashcardPageV5({
  ...
  const stats = useMemo(() => {
    ...
    const mastered = items.filter((it: { id: string }) => masteredSet.has(it.id)).length;
    const due = items.filter((it: { id: string }) => dueSet.has(it.id)).length;
    const newN = items.filter((it: { id: string; lastReviewedAt?: number }) =>
      !masteredSet.has(it.id) && !dueSet.has(it.id) && !it.lastReviewedAt
    ).length;
    const learning = items.filter((it: { id: string; lastReviewedAt?: number }) =>
      !masteredSet.has(it.id) && !dueSet.has(it.id) && !!it.lastReviewedAt
    ).length;
    const pct = total > 0 ? Math.round((mastered / total) * 100) : 0;
    return { total, mastered, due, newN, learning, pct };
  ...
  const pickModalPool = useCallback((): StudyCard[] => {
    ...
    if (activeTab === 'words') {
      ...
      switch (modalMode) {
        case 'revise':
          // SEULEMENT les dues, triées par ancienneté (plus en retard = priorité).
          pool = src
...
function StatGrid({
  ...
  stats: { total: number; mastered: number; learning: number; newN: number; due: number; pct: number };
  copy: CopyType;
}) {
  return (
    <div className="fc5-stats-grid">
      <div className="fc5-stat-card">
        <div className="fc5-stat-value fc5-stat-mastered">{stats.mastered.toLocaleString()}</div>
        <div className="fc5-stat-label">{copy.statMastered}</div>
      </div>
      <div className="fc5-stat-card">
        <div className="fc5-stat-value fc5-stat-learning">{stats.learning.toLocaleString()}</div>
        <div className="fc5-stat-label">{copy.statLearning}</div>
      </div>
      <div className="fc5-stat-card">
        <div className="fc5-stat-value fc5-stat-new">{stats.newN.toLocaleString()}</div>
        <div className="fc5-stat-label">{copy.statNew}</div>
      </div>
      <div className="fc5-stat-card">
        <div className={`fc5-stat-value fc5-stat-due ${stats.due > 0 ? 'is-warn' : ''}`}>

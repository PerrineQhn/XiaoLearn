...
export interface LessonV2Exercise {
  ...
  /**
   * Pour dialogue-response avec N tours vides : fournit choices/correctIndex
   * pour chaque sous-étape. steps[0] correspond au 1er tour vide, etc.
   * Si absent, exercise.choices/correctIndex s'appliquent à toutes les étapes.
   */
  steps?: Array<{ choices: string[]; correctIndex: number; explanation?: string; explanationEn?: string }>;
  /**
...
const DialogueResponseCard = ({
  exercise,
  language,
  answered: answeredExt,
  selectedIndex: selectedExt,
  onSelect,
  onValidate,
  onNext,
  lessonTitle
  ...
  // Indices des tours utilisateur vides (à compléter)
  const emptyTurnIndices = lines.reduce<number[]>(
    (acc, l, i) => (l.isUser && !l.hanzi ? [...acc, i] : acc), []
  );
  const isMultiStep = emptyTurnIndices.length > 1;

  // État interne pour le mode multi-étapes
  const [subStep, setSubStep] = useState(0);
  const [internalSelected, setInternalSelected] = useState<number | null>(null);
  const [internalAnswered, setInternalAnswered] = useState(false);
  const [prevChoices, setPrevChoices] = useState<string[]>([]);
  const [allStepsCorrect, setAllStepsCorrect] = useState(true);

  // En mode simple, on utilise l'état externe ; en multi-step, l'interne
  const answered    = isMultiStep ? internalAnswered : answeredExt;
  const selectedIndex = isMultiStep ? internalSelected : selectedExt;

  // Choices/correctIndex pour la sous-étape courante
  const currentTargetIdx = emptyTurnIndices[subStep] ?? (() => {
    // Fallback V22 : dernier tour vide ou premier isUser
    const idx = [...lines].reverse().findIndex((l) => l.isUser && !l.hanzi);
    return idx >= 0 ? lines.length - 1 - idx : lines.findIndex((l) => l.isUser);
  })();
  const currentStepDef   = exercise.steps?.[subStep];
  const currentChoices   = currentStepDef?.choices   ?? exercise.choices;
  const currentCorrect   = currentStepDef?.correctIndex ?? exercise.correctIndex;
  const currentExpl      = (language === 'en' && (currentStepDef as any)?.explanationEn)
    ? (currentStepDef as any).explanationEn
    : currentStepDef?.explanation
      ?? (language === 'en' && exercise.explanationEn ? exercise.explanationEn : exercise.explanation);
  const isLastSubStep = subStep >= emptyTurnIndices.length - 1;

  // Dialogue visible : limité au tour cible actuel, tours précédents injectés
  const visibleLines = lines
    .slice(0, currentTargetIdx + 1)
    .map((l, i) => {
      const stepIdx = emptyTurnIndices.indexOf(i);
      if (stepIdx >= 0 && stepIdx < subStep && prevChoices[stepIdx]) {
        return { ...l, hanzi: prevChoices[stepIdx], pinyin: '', translationFr: '', translationEn: '' };
      }
      return l;
    });
  const beforeLast = visibleLines.slice(0, visibleLines.length - 1);
  const lastLine   = visibleLines[visibleLines.length - 1];

  const isCorrect = selectedIndex !== null && selectedIndex === currentCorrect;
  ...
  const playLine = useCallback((line: DialogueLine) => {
    if (line.audio) { playAudioWithFallback(line.audio).catch(() => {}); return; }
    if (line.hanzi) { playHanziAudio(line.hanzi).catch(() => {}); }
  }, []);

  function handleSelect(idx: number) {
    if (isMultiStep) setInternalSelected(idx);
    else onSelect(idx);
  }
  ...
  function handleValidate() {
    if (isMultiStep) {
      ...
      setAllStepsCorrect(prev => prev && isCorrect);
    } else {
      onValidate(isCorrect);
    }
  }
  ...
  function handleNext() {
    if (isMultiStep && !isLastSubStep) {
      ...
      setPrevChoices(p => [...p, selectedIndex !== null ? currentChoices[selectedIndex] : '']);
      setSubStep(s => s + 1);
      setInternalSelected(null);
      setInternalAnswered(false);
    } else {
      // Dernière sous-étape : reporter le résultat au parent
      if (isMultiStep) {
        const finalCorrect = allStepsCorrect && isCorrect;
        onValidate(finalCorrect);
      }
      onNext();
    }
  }
    ...
    <div className="lv2-exercise lv2-exercise--dialogue">
      ...
      <div className="lv2-dialogue-chat">
            ...
            <div className="lv2-dialogue-content">
              <div className="lv2-dialogue-hanzi">
                ...
                {line.audio || line.hanzi ? (
                  <button type="button" className="lv2-dialogue-audio" onClick={() => playLine(line)} aria-label="Play">
                    🔊
                  </button>
                ) : null}
              </div>
        ...
        {lastLine && (
          ...
          >
            ...
            <div className="lv2-dialogue-content">
              {answered ? (
                <div className="lv2-dialogue-hanzi">{currentChoices[currentCorrect]}</div>
              ) : (
      ...
      <div className="lv2-exercise-choices" role="radiogroup">
        {currentChoices.map((choice, idx) => {
          let stateClass = '';
          if (answered) {
            if (idx === currentCorrect) stateClass = 'lv2-choice--correct';
            else if (idx === selectedIndex) stateClass = 'lv2-choice--wrong';
            else stateClass = 'lv2-choice--dimmed';
          } else if (idx === selectedIndex) {
            stateClass = 'lv2-choice--selected';
      ...
      <div className="lv2-exercise-actions">
        {!answered ? (
          ...
          <button type="button" className="lv2-btn lv2-btn--primary" onClick={handleNext}>
            {isMultiStep && !isLastSubStep
              ? (language === 'en' ? 'Next reply' : 'Réplique suivante')
              : getCopy(language, 'next')
            } →
          </button>
        )}
      ...
      {answered && (
        <div className={`lv2-exercise-feedback ${isCorrect ? 'is-correct' : 'is-wrong'}`}>
          ...
          {currentExpl && (
            <div className="lv2-feedback-explanation">
              {renderMarkdownInline(currentExpl)}
            </div>
          )}
          {!isCorrect && lessonTitle && (
            <button
              ...
              onClick={() =>
                dispatchAskAiWhy({
                  ...
                  lessonTitle,
                  prompt: promptText || '',
                  userAnswer: selectedIndex !== null ? currentChoices[selectedIndex] : '',
                  correctAnswer: currentChoices[currentCorrect]
                })
              }
            >
              {language === 'en' ? 'Ask Prof. Xiao: why?' : 'Demander à Prof. Xiao : pourquoi ?'}

...
export default function LectureReaderScreen() {
  ...
  /*
    Sur grand écran, le texte et les questions cohabitent. Répondre à un
    exercice de compréhension sans avoir le passage sous les yeux teste la
    mémoire, pas la compréhension : l'onglet était une contrainte de place,
    pas un choix pédagogique. La place existant ici, les onglets disparaissent
    et la sélection d'onglet n'a plus d'effet sur ce qui est visible.
  */
  const split = wide && quizPhase !== 'result';
  const showText = wide ? quizPhase !== 'result' : activeTab === 'text';
  const showQuiz = wide ? quizPhase !== 'result' : (activeTab === 'quiz' && quizPhase === 'quiz');
  const showResult = quizPhase === 'result' && (wide || activeTab === 'quiz');

  // ── Colonne texte ──
  const textPane = (
    <ScrollView contentContainerStyle={[s.textBody, readableContent]}>
      ...
      {lecture.text.split('\n').map((line, i) => {
        ...
        return (
          <View key={i} style={[s.lineBlock, { borderLeftColor: accent + '40' }]}>
            <Text selectable style={[s.lineZh, { color: c.textPrimary }]}>{zhPart}</Text>
            {showPinyinLocal && pinyinPart !== '' && (
              <Text selectable style={[s.linePinyin, { color: accent }]}>{pinyinPart}</Text>
            )}
            {showTranslation && transl !== '' && (
              <Text selectable style={[s.lineFr, { color: c.textSecondary }]}>{transl}</Text>
            )}
          </View>
        );
      })}

      {/* CTA Quiz — sans objet en vue jumelée, où le quiz est déjà à l'écran */}
      {!split && (
        <TouchableOpacity
  ...
  const quizPane = !question ? null : (
    <>
      ...
      <ScrollView contentContainerStyle={[s.quizBody, readableContent]}>
  ...
  // ── Résultat : jamais en colonne, c'est un bilan qui se lit d'un bloc ──
  const resultPane = (
    <ScrollView contentContainerStyle={[s.resultBody, readableContent]}>
      {(() => {
        ...
        return (
          <>
            ...
            <View style={s.resultActions}>
              ...
              {/* « Relire » ne mène nulle part en vue jumelée : recommencer
                  ramène déjà le texte et les questions côte à côte. */}
              {!wide && (
                <TouchableOpacity
  ...
  return (
    <SafeAreaView style={[s.root, { backgroundColor: c.appBg }]}>
      ...
      {/* Tab bar — masquée dès que les deux volets tiennent ensemble */}
      {!wide && (
      ...
      {split ? (
        <View style={s.splitRow}>
          <View style={s.pane}>{textPane}</View>
          <View style={[s.pane, s.paneRight, { borderLeftColor: c.borderLight }]}>{quizPane}</View>
        </View>
      ) : (
        <>
          {showText && textPane}
      ...
      <AskXiaoBubble prompt={t('xiao.askPrefill', { title: lecture.titleZh })} />
    </SafeAreaView>
  );
}

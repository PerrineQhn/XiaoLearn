...
export const getDialogueById = (id: string) =>
  dialogues.find(d => d.dialogue.id === id)?.dialogue;

export const getDialoguesByLevel = (level: DialogueLevel) =>
  dialogues.filter(d => d.cecrLevel === level);

/** Niveaux réellement présents, dans l'ordre d'affichage. */
export const availableLevels = (): DialogueLevel[] =>
  DIALOGUE_LEVELS.filter(l => dialogues.some(d => d.cecrLevel === l));
